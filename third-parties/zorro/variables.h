...
#define LogTrainRun g->nLogCycle
#define Pin			g->nPin
...
#define NumOutliers g->numOutliers
...
#define AssetPrev		g->sAssetPrev
#define AssetBar		g->asset->nBar
#define AssetFirstBar g->asset->nFirstPriceBar
#define AssetFrame	g->asset->nFrame
#define AssetZone		g->asset->nZone
#define AssetFrameZone g->asset->nZone
#define AssetMarket	g->asset->nMarket
#define AssetMarketStart g->asset->nStartMarket
#define AssetMarketEnd g->asset->nEndMarket
#define AssetMarketZone g->asset->nMarketZone
...
#define AssetPtr		((void**)AssetVar)
#define AssetFactor	g->vAssetFactor
#define AlgoVar2		(g->statLong->Skill)
#define AlgoVar		(g->statShort->Skill)
#define AlgoInt		((int*)AlgoVar)
#define AlgoPtr		((void**)AlgoVar)
#define AlgoPtr2		((void**)AlgoVar2)
...
#define SourceTrade	g->asset->sSourceTrade
#define SourceLive	g->asset->sSourceLive
#define SourceHist	g->asset->sSourceHist
#define Component		g->statLong->nComponent
#define SeriesBuffer	g->pSeriesBuffer

#define AssetMode		g->asset->flags
#define PriceOffset	(g->asset->vOffset)
#define PriceFactor	(g->asset->vFactor)
...
#define priceO			priceOpen
#define priceH			priceHigh
#define priceL			priceLow
#define priceC			priceClose
...
#define ReturnLR			g->w.vLinResult
#define ReturnCBI			g->w.vCBI
#define AEDepth			(g->w.vEquityPeak-g->vEquity)
#define AEDays				(max(0,g->nBar-g->w.nEquityPeakBar)*g->vBarPeriod/1440.)
...
#define ResultsAtBar		(g->pResults)
#define ResultsDaily		(g->pCurve)
#define ProfileAtBar		(g->pEquity)
#define DrawDownAtBar	(g->pDrawDown)
#define DatesDaily		(g->pTime)
#define Bars				(g->bars)
#define Shape				(g->pShape)
#define ScholzBrake		g->vScholzBrake
#define SeriesLength		g->nSeriesLength
...
#define TradeEntryTime	g->tr->nEntryTime
...
#define TradeInt			((int*)TradeVar)
#define TradeStr			(string)&(g->tr->Skill)
#define TradeParameter	(g->tr->fArg)
#define TradeDir			ifelse(TradeIsShort,-1.,1.)
#define TradeFill			(TradePriceOpen-ifelse(TradeIsShort,TradeSpread,0))
#define TradeValue		(TradePriceClose-ifelse(TradeIsLong,TradeSpread,0))
...
#define TradeUUID			((char*)(g->tr->vExtraData))
...
#define ref(x,n)			(series(x,abs(n+1)))[abs(n)]
...
#define HMS	"%H:%M:%S"
#define YMD	"%Y%m%d"
#define OHLC seriesO(),seriesH(),seriesL(),seriesC()

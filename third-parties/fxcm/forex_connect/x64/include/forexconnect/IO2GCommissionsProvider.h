...
public:
    ...
    virtual int getChangeNumber() = 0;

    virtual double getDiscountAmount() = 0;
};
...
class IO2GCommissionsProvider : public IAddRef
{
 ...
 public:
    ...
    virtual double calcOpenCommission(IO2GOfferTableRow * offer, IO2GAccountTableRow* account, int amount, const char* buySell, double rate) = 0;
    virtual double calcCloseCommission(IO2GOfferTableRow * offer, IO2GAccountTableRow* account, int amount, const char* buySell, double rate) = 0;
    virtual double calcTotalCommission(IO2GOfferTableRow * offer, IO2GAccountTableRow* account, int amount, const char* buySell, double rateOpen, double rateClose) = 0;
};
